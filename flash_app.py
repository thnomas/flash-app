...
def quiz():
    ...
    if not cards or len(cards) < 3:
        ...
        quiz_session = random.sample(cards, 3)
...
def review(cards):
    ...
    if not cards:
        ...
        print("Let's Review!")
        # quiz_session = random.shuffle(words, 1)
...
while True:
    choice = input(yellow_color + "(0) Review / (1) Quiz / (2) Add cards / (3) Stats / (4) List cards / (5) Exit"  + reset_text + "\n>>> " )
    if choice == "0":
        review(cards)
    elif choice == "1":
        quiz()
